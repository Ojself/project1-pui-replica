...
let play_sketch_dance_floor = function(p) {
  ...
  p.preload = function() {
    game.sound = p.loadSound(
      `assets/${getCookie('pistaMusic')}.mp3`,
      soundLoaded
    );
  };
...
if (displayGameDanceFloor) {
  ...
  //P5
  play_sketch = new p5(
    play_sketch_dance_floor,
    document.getElementById('game-dance-floor')
  );
}

// Takes the user to a youtube video that plays happy birthday
function playBdaySong() {
  const url = 'https://www.youtube.com/watch?v=fTbEpGZyseA';
  window.location = url;
}
